...
void Misc::showKeybinds() noexcept
{
    ...
    ImGui::Begin("按键绑定列表", nullptr, windowFlags);
    ImGui::PopStyleVar();
...
void Misc::spectatorList() noexcept
{
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_WindowTitleAlign, { 0.5f, 0.5f });
    ImGui::Begin("观战者列表", nullptr, windowFlags);
    ImGui::PopStyleVar();
...
void Misc::watermark() noexcept
{
    ...
    ImGui::Text("BSK [%s] | %d FPS | %d MS | %s | %s%s%s",
#ifdef DEBUG
        "DEBUG",
#else
        "RELEASE",
        ...
        team == Team::Spectators ? "选队" : team == Team::TT ? "T" : team == Team::CT ? "CT" : "无",
        AntiAim::moving_flag_text[AntiAim::latest_moving_flag], inReload ? " | 换弹中" : "",
        noScope ? " | 没开镜" : "");
    static int damage{};
    ...
    ImGui::Text("伤害 %d | 命中率 %d%% | 最小伤害 %d | 解析 %s | 命中/未命中 %d/%d (%.2lf%%)",
        damage,
        hit_chance,
        min_damage,
        resolver ? "开" : "关",
        ...
        resolver::misses,
        resolver::hit_rate);
    ImGui::Text("%s %s | 角度 %d%s%s%s | 目标 %s",
        ...
        AntiAim::lby_mode_text[config->fakeAngle[AntiAim::latest_moving_flag].enabled
                ...
                config->tickbase.hideshots.isActive() ? " | 藏头射击" : "",
                config->tickbase.teleport && (config->tickbase.doubletap.isActive() || config->tickbase.hideshots.isActive()) ? " | 闪现" : "",
...
void Misc::drawBombTimer() noexcept
{
    ...
    if (drawDamage) {
        ...
        if (health <= (truncf(finalBombDamage * 10) / 10)) {
            ...
            ImGui::textUnformattedCentered("会被炸死");
            ImGui::PopStyleColor();
        }
        else {
            std::ostringstream text; text << "爆炸伤害: " << std::clamp(displayBombDamage, 0, health - 1);
    ...
    if (plantedC4.defuserHandle != -1) {
        ...
        if (plantedC4.defuserHandle == GameData::local().handle) {
            if (canDefuse) {
                ...
                ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(255, 0, 0, 255));
                ImGui::textUnformattedCentered("你不能拆包!");
            }
            ImGui::PopStyleColor();
...
void Misc::hurtIndicator() noexcept
{
    ...
    ImGui::SetNextWindowSizeConstraints({ 0, -1 }, { FLT_MAX, -1 });
    ImGui::Begin("减速指示器", nullptr, ImGuiWindowFlags_NoTitleBar | (gui->isOpen() ? 0 : ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoDecoration));
